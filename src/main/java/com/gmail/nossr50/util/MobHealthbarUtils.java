...
public final class MobHealthbarUtils {
    ...
    public static void handleMobHealthbars(LivingEntity target, double damage, mcMMO plugin) {
        ...
        if (displayTime != -1) {
            ...
            mcMMO.p.getFoliaLib().getImpl().runAtEntityLater(target, new MobHealthDisplayUpdaterTask(target), 1 + ((long) displayTime * Misc.TICK_CONVERSION_FACTOR)); // Clear health display after 3 seconds
        }
    }
