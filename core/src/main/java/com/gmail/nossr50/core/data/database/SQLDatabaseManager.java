...
public final class SQLDatabaseManager implements DatabaseManager {
    ...
    private void checkDatabaseStructure(Connection connection, UpgradeType upgrade) {
        ...
        try {
            ...
            switch (upgrade) {
                case UpgradeType.ADD_FISHING:
                    checkUpgradeAddFishing(statement);
                    break;

                case UpgradeType.ADD_BLAST_MINING_COOLDOWN:
                    checkUpgradeAddBlastMiningCooldown(statement);
                    break;

                case UpgradeType.ADD_SQL_INDEXES:
                    checkUpgradeAddSQLIndexes(statement);
                    break;

                case UpgradeType.ADD_MOB_HEALTHBARS:
                    checkUpgradeAddMobHealthbars(statement);
                    break;

                case UpgradeType.DROP_SQL_PARTY_NAMES:
                    checkUpgradeDropPartyNames(statement);
                    break;

                case UpgradeType.DROP_SPOUT:
                    checkUpgradeDropSpout(statement);
                    break;

                case UpgradeType.ADD_ALCHEMY:
                    checkUpgradeAddAlchemy(statement);
                    break;

                case UpgradeType.ADD_UUIDS:
                    checkUpgradeAddUUIDs(statement);
                    return;

                case UpgradeType.ADD_SCOREBOARD_TIPS:
                    checkUpgradeAddScoreboardTips(statement);
                    return;

                case UpgradeType.DROP_NAME_UNIQUENESS:
                    checkNameUniqueness(statement);
                    return;

                case UpgradeType.ADD_SKILL_TOTAL:
                    checkUpgradeSkillTotal(connection);
                    break;
                case UpgradeType.ADD_UNIQUE_PLAYER_DATA:
